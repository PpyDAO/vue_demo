<template>
  ...
  >
    <div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles, bgStyle]">
      ...
      <div ref="endtip" v-if="isRange" class="endSideTip" :style="sideTipStyle">{{val[1]}}</div>
      <div
        ref="process"
        :class="['vue-slider-process', { 'vue-slider-process-dragable': isRange && processDragable }]"
        :style="processStyle"
  ...
  export default {
    ...
    methods: {
      ...
      setTransform (val, isIdleSlider) {
        ...
        let processEndSize = this.height - processEndPos - this.dotSize
    ...
    mounted () {
      this.$refs.endtip.style.bottom = `${this.height + 10}px`
